...
;; (defun sly-loc-defun-p (loc) (eq (sly-loc-type loc) 'swank-backend::defun))
;; (defun sly-loc-defvar-p (loc) (eq (sly-loc-type loc) 'swank-backend::defvar))
  ...
  (let* ((tmp (mapcar (lambda (x)
    ...
    (dolist (method methods)
      (let* ((dspec (first method))
        ...
        (setq specs (log4cl:fix-method-spec-list specs))
               ...
               (find-logger ()
                 (cond ((and file (not rest))
                       ...
                       (t
                        ...
                        (values 
                         (log4cl:%get-logger package-categories
                                      (category-separator)
                                      (category-case)
                                      nil nil
                                      nil nil nil
